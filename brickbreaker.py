...
PADDLE_HEIGHT = 15
BALL_RADIUS =10
LIVES_FONT= pygame.font.SysFont("comicsans", 20)    
...
class Brick:
    ...
    def collide(self, ball):        
        if not(ball.x >= self.x and ball.x <= self.x + self.width):
            return False
    ...
    def interpolate(color_a, color_b, t):
        #color_a and color b are rgb tuples
...
def ball_paddle_collision(ball,paddle):
    if not(ball.x >= paddle.x and ball.x <= paddle.x + paddle.width):
        return
...
def main():
    ...
    def display_text(text):
        ...
        render_text_width = WIDTH/2 - text_render.get_width()/2
        render_text_height = HEIGHT/2 - text_render.get_height()/2
        win.blit(text_render, (render_text_width, render_text_height))
        pygame.display.update()
